...
public class MenuView{
	...
	*/
	public void printMenu(){
	     ...
	     while(true){
           System.out.println("----------------------------------------------------------------------------------");
           System.out.println("1. 전체검색    2. 모델번호검색     3.등록     4. 수정   5. 삭제    6. 가격수정    7. 가격순 정렬    9. 종료");
		   System.out.println("----------------------------------------------------------------------------------");
		   ...
		   switch(menu){
	           ...
	           break;
			   case "6" : 
	             this.inputPriceUpdate();
			   break;
			   case "7" : 
				   controller.getModelSortedByPrice();
			   break;
			   case "9" : 
                 System.exit(0);
			   default:
  ...
  /**
   * 모델번호에 해당하는 전자제품 가격 변경하기 위한 키보드 입력 처리하는 메소드
   */
  public void inputPriceUpdate() {
	  System.out.println("수정하려는 전자제품 모델번호는??");
	  int modelNo = Integer.parseInt(sc.nextLine());
	  
	  System.out.println("변경하려는 모델 가격은??");
	  int modelPrice = Integer.parseInt(sc.nextLine());
	  
	  controller.updatePrice(new Electronics(modelNo, modelPrice));
  }
