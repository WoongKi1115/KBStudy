...
SELECT SUBSTR(이메일 ,1, INSTR(이메일, '@')-1), SUBSTR(이메일 ,INSTR(이메일, '@')+1)
FROM TEACHER;
...
-- CEIL, FLOOR : 결과가 정수로 반환(자리수 설정 없음)
...
SELECT EMPNO, ENAME, MGR  ,NVL2(MGR, MGR||'는 관리자','최고관리자')
 ...
 -- NULLIF는 첫번 째 인수와 두번 째 인수가 같으면 NULL 아니면 첫번 째 인수를 반환.
...
DECODE(JOB, 'MANAGER', SAL * 0.1, 'ANALYST', SAL * 0.2, 'SALESMAN', SAL * 0.3) AS 성과급
FROM EMP;
